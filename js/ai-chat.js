...
import { speak, startListening, stopListening, isRecognitionSupported, getVoiceLang, toggleVoiceLang } from './speech.js';
...
const BASE_SYSTEM = `あなたは日本語会話パートナーです。生徒は台湾人ビジネスパーソン（N4〜N3レベル）。

【最重要ルール：出力フォーマット】
毎回のメッセージは必ずこの形式で出力してください：
- 日本語を1〜2文だけ書く
- 改行して「→」の後に繁體中文の翻訳を書く
- それ以上書かない
...
【ルール】
1. 毎回1〜2文のみ。長い説明は絶対しない。
2. 生徒が間違えた場合のみ、✗と✓で短く訂正し、1行で中文説明。
3. 生徒が中国語で書いたら、日本語でどう言うかを1文で示す。
4. 文法説明は不要（訂正の1行中文のみ）。
5. 会話のテンポを重視。質問を投げて相手に話させる。
6. 難しい漢字にはふりがなを（）で付ける。`;
...
function formatMessage(text) {
    return text
        ...
        .replace(/^(→.*)$/gm, '<span class="ai-translation">$1</span>')
        .replace(/^(✗.*)$/gm, '<span class="ai-wrong">$1</span>')
        .replace(/^(✓.*)$/gm, '<span class="ai-correct">$1</span>')
        .replace(/\n/g, '<br>');
}
    ...
    <div class="view ai-chat-view">
        ...
        <div class="ai-chat-input-area">
            <form class="ai-chat-form" onsubmit="event.preventDefault(); window.JT.sendAIMessage()">
                ...
                <button type="button" class="ai-lang-btn" onclick="window.JT.switchVoiceLang()" title="切換辨識語言">
                    ${getVoiceLang() === 'ja-JP' ? 'JP' : '中'}
                </button>
